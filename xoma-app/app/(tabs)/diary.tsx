...
export default function DiaryScreen() {
  ...
  const hasSelectedDateEntry = useMemo(() => {
    return diaryEntries.some((e) => isSameLocalDay(fromYYYYMMDDLocal(e.date), selectedDate));
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          ...
          <View className="flex-row mb-4">
            ...
            >
              <Image 
                ...
                resizeMode="contain"
                style={{ width: 16, height: 16, marginRight: 8, tintColor: '#2D5A6E' }}
              />
              <Text className="text-primary font-bold text-sm mr-2">Selecciona una fecha</Text>
            </TouchableOpacity>
          </View>
        ...
        <View className="px-6 pb-32">
            ...
            <View className="bg-[#F8FAF9] rounded-[32px] p-8 items-center border border-gray-100 shadow-sm mb-10 relative overflow-hidden">
              ...
              <Link href="/diary-form" asChild>
                <TouchableOpacity className="bg-primary px-8 py-4 rounded-full shadow-sm flex-row items-center">
                  ...
                  <Text className="text-white font-bold text-base">Crear registro</Text>
                </TouchableOpacity>
              </Link>
            </View>
          )}
          ...
          {filteredEntries.map((entry) => (
            <TouchableOpacity key={entry.id} onPress={() => router.push(`/diary-form?diaryId=${entry.id}`)} className="bg-white rounded-[32px] overflow-hidden mb-5 shadow-sm border border-gray-100 p-6">
              ...
              <View className="flex-row justify-between items-start mb-2">
                ...
                <View className={`px-4 py-2 rounded-full flex-row items-center ${
                  entry.riskLevel === 'low' ? 'bg-[#CFF1E2]' : 
                  entry.riskLevel === 'moderate' ? 'bg-[#FEF0C7]' : 'bg-[#FEE2E2]'
                }`}>
                  <Text className={`mr-1 text-xs ${
                    entry.riskLevel === 'low' ? 'text-primary' : 
                    entry.riskLevel === 'moderate' ? 'text-orange-700' : 'text-red-700'
                  }`}>
                    {entry.riskLevel === 'low' ? '🌿' : entry.riskLevel === 'moderate' ? '⚠️' : '🚨'}
                  </Text>
                  <Text className={`font-bold text-xs ${
                    entry.riskLevel === 'low' ? 'text-primary' : 
                    entry.riskLevel === 'moderate' ? 'text-orange-700' : 'text-red-700'
                  }`}>
                    {entry.riskLevel === 'low' ? 'Calma' : entry.riskLevel === 'moderate' ? 'Alerta' : 'Crítico'}
                  </Text>
                </View>
              </View>
              ...
              <Text className="text-xl font-bold text-gray-800 mb-3">
                {entry.practiceSkills.length > 0 ? entry.practiceSkills[0] : 'Registro de diario'}
              </Text>

              {/* Extracto de texto (simulado por ahora) */}
              <Text className="text-gray-500 text-sm mb-6 leading-relaxed" numberOfLines={2}>
                Hoy pude practicar los ejercicios de respiración que aprendí. Me sentí mucho...
              </Text>
          ...
          <View className="flex-1 justify-center items-center bg-black bg-opacity-20">
              ...
              <View className="flex-row justify-between items-center mb-4">
                ...
                <TouchableOpacity
                  onPress={() => {
                    ...
                    const candidate = new Date(calendarMonth);
                    candidate.setMonth(candidate.getMonth() + 1);
                    const isFutureMonth = candidate.getFullYear() > today.getFullYear()
            ...
            <View className="flex-row mb-2">
              {['D', 'L', 'M', 'X', 'J', 'V', 'S'].map((day, index) => (
                <View key={index} className="flex-1 items-center p-2">
                  <Text className="text-gray-400 text-sm w-8 text-center font-bold">{day}</Text>
                </View>
              ))}
            </View>
            ...
            <View className="flex-row flex-wrap">
              {generateCalendarDays(calendarMonth).map((day, index) => {
                ...
                const todayMid = new Date(today);
                todayMid.setHours(0, 0, 0, 0);
                const candidate = day ? new Date(calendarMonth.getFullYear(), calendarMonth.getMonth(), day) : null;
                ...
                const isSelected = !!candidate && isSameLocalDay(candidate, selectedDate);
                const isToday = !!candidate && isSameLocalDay(candidate, today);
                  ...
                  >
                    <View
                      ...
                      style={{
                        ...
                        borderRadius: 9999,
                        overflow: 'hidden',
                        backgroundColor: isSelected ? '#2D5A6E' : (isToday ? '#8FB9A8' : 'transparent'),
                      }}
                    >
                      <Text className={`text-sm font-medium ${
                        isSelected || isToday
                          ? 'text-white'
                          : day ? (isFuture ? 'text-gray-300' : 'text-gray-600') : 'text-transparent'
                      }`}>
                        {day || ''}
                      </Text>
                    </View>
